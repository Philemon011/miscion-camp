...
function CountdownUnit({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center flex-1 min-w-0">
      <div className="relative w-full aspect-square max-w-[80px] bg-white rounded-2xl shadow-sm flex items-center justify-center overflow-hidden">
        <AnimatePresence mode="popLayout">
          <motion.span
            key={value}
            initial={{ y: 16, opacity: 0 }}
            ...
            exit={{ y: -16, opacity: 0 }}
            transition={{ duration: 0.4, ease: "easeOut" }}
            className="absolute font-display text-dark text-2xl md:text-4xl"
          >
            {String(value).padStart(2, "0")}
          </motion.span>
        </AnimatePresence>
      </div>
      <span className="mt-2 font-body text-[10px] md:text-sm uppercase tracking-widest text-dark/60 text-center">
        {label}
      </span>
    </div>
  );
  ...
  return (
    <section id="countdown" className="bg-cream px-6 py-20 md:py-28">
      ...
      >
        ...
        <motion.div variants={item} className="flex items-start justify-center gap-3 md:gap-6 mt-12 w-full">
          <CountdownUnit value={time.days} label="Jours" />
          <CountdownUnit value={time.hours} label="Heures" />
          <CountdownUnit value={time.minutes} label="Minutes" />
          <CountdownUnit value={time.seconds} label="Secondes" />
