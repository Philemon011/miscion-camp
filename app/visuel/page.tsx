...
export default function VisuelPage() {
  ...
  const [photoDims, setPhotoDims] = useState({ w: 0, h: 0 });
  const [scale, setScale] = useState(1);
  // Taille réelle du conteneur d'aperçu, tenue à jour via ResizeObserver.
  // Lire containerRef.current?.offsetWidth directement dans le corps du
  // composant n'est PAS réactif : au tout premier render après l'apparition
  // du conteneur (juste après l'upload), React n'a pas encore eu de passe
  // de re-render déclenchée par ce changement, donc l'image restait
  // invisible jusqu'au prochain événement (ex: un toucher). En stockant la
  // taille dans un state mis à jour par ResizeObserver, l'apparition du
  // conteneur déclenche immédiatement un re-render avec la bonne taille.
  ...
  // Suit les changements de taille du conteneur d'aperçu (resize fenêtre,
  // rotation mobile...) en complément de la mesure initiale faite dans le
  // ref callback au montage.
  useEffect(() => {
  ...
  // Rect d'aperçu recalculé à chaque render à partir de la même fonction.
  // containerSize vient du state (mis à jour au montage + ResizeObserver),
  // pas d'une lecture directe de containerRef.current, qui n'est pas réactive.
  const previewRect =
    photoSrc && photoDims.w && containerSize
      ? computeDrawRect(photoDims.w, photoDims.h, containerSize)
      : null;
            ...
            >
              ...
              <div
                ref={(node) => {
                  ...
                  if (!node) return;
                  // Mesure immédiate au montage (évite l'attente du premier
                  // événement pour afficher l'image après upload).
